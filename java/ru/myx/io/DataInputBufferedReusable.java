...
/**
 * Fast, highly buffered, contains additional control methods. Dedicated for
 * wrapping input streams.
 * 
 * @author myx
 * 
 *         READ_AHEAD = 4k
...
public final class DataInputBufferedReusable extends InputStream implements DataInput {
	private static final int			BUFFER_AHEAD	= Engine.MODE_SIZE
																? 1 * 1024
																: 4 * 1024;
	...
	private final byte[]				buffer			= new byte[DataInputBufferedReusable.BUFFER_CAPACITY];
	
	private final char[]				chararr			= new char[DataInputBufferedReusable.BUFFER_UTF];
	...
	private final void fill() throws IOException {
		...
		if (this.readLimitSet) {
			read = this.in.read( this.buffer, 0, this.readLimit < DataInputBufferedReusable.BUFFER_CAPACITY
					? this.readLimit
					: DataInputBufferedReusable.BUFFER_CAPACITY );
			if (read != -1) {
				this.readLimit -= read;
				if (this.readLimit <= 0) {
			...
			read = this.in.read( this.buffer, 0, DataInputBufferedReusable.BUFFER_AHEAD );
		}
		this.count = read;
	}
	...
	public final int readInt() throws IOException {
		if (this.count - this.pos >= 4) {
			return ((this.buffer[this.pos++] & 0xff) << 24)
					+ ((this.buffer[this.pos++] & 0xff) << 16)
					+ ((this.buffer[this.pos++] & 0xff) << 8)
	...
	public final long readLong() throws IOException {
		if (this.count - this.pos >= 8) {
			return ((long) (this.buffer[this.pos++] & 0xff) << 56)
					+ ((long) (this.buffer[this.pos++] & 0xff) << 48)
					+ ((long) (this.buffer[this.pos++] & 0xff) << 40)
					+ ((long) (this.buffer[this.pos++] & 0xff) << 32)
					+ ((long) (this.buffer[this.pos++] & 0xff) << 24)
					+ ((long) (this.buffer[this.pos++] & 0xff) << 16)
					+ ((long) (this.buffer[this.pos++] & 0xff) << 8)
					+ (this.buffer[this.pos++] & 0xff);
		}
		return ((long) this.readUnsignedByte() << 56)
				+ ((long) this.readUnsignedByte() << 48)
				+ ((long) this.readUnsignedByte() << 40)
				+ ((long) this.readUnsignedByte() << 32)
				+ ((long) this.readUnsignedByte() << 24)
				+ (this.readUnsignedByte() << 16)
				+ (this.readUnsignedByte() << 8)
				+ ((this.readUnsignedByte() & 0xff) << 0);
	}
	...
	public final String readUTF() throws IOException {
		...
		final char[] chararr = utflen <= DataInputBufferedReusable.BUFFER_UTF
				? this.chararr
				: new char[utflen];
	...
	public final void setBytes(final byte[] bytes) {
		this.readLimitSet = false;
		this.pos = 0;
		if (bytes.length < DataInputBufferedReusable.BUFFER_CAPACITY) {
	...
	public final void setBytes(final byte[] bytes, final int offset, final int length) {
		this.readLimitSet = false;
		this.pos = 0;
		if (length < DataInputBufferedReusable.BUFFER_CAPACITY) {
			this.in = DataInputBufferedReusable.DUMMY_STREAM;
			this.count = length;
