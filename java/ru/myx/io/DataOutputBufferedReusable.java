...
/** Fast, highly buffered, contains additional control methods. Supports both - wrapping DataOutput
 * object and OutputStream object.
 *
 * Special method - getWrittenByteCount() returns amount of bytes written since last setStream or
 * setOutput method was called.
 *
 * Special method - writeFullyFromStream() writes all bytes from given InputStream using own buffers
 * to make this transfer more effective.
 *
 * UTF buffer used to try not to traverse string twice for calculating byte size before flushing to
 * stream.
 *
 * @author myx
 *
 ...
 *
 *         TOTAL_BUFFERS = 72k */
public final class DataOutputBufferedReusable extends OutputStream implements DataOutput {
	...
	private DataOutput outOutput;

	private int pos;

	private int writtenDown;

	/**
	 *
	 */
	public DataOutputBufferedReusable() {

		this.buffer = new byte[32 * 1024];
		this.bytearr = new byte[4 * 1024];
	}
	/** Creates an instance of reusable buffered data input.
	 *
	 * @param byteBufferCapacity
	 *            32 * 1024 .. 64 * 1024
	 * @param utfBufferCapacity
	 *            4 * 1024 .. 8 * 1024 */
	public DataOutputBufferedReusable(final int byteBufferCapacity, final int utfBufferCapacity) {
		
		this.buffer = new byte[byteBufferCapacity];
		this.bytearr = new byte[utfBufferCapacity];
	}
	...
	public final void close() throws IOException {
		...
		if (this.outStream != null) {
			if (this.pos > 0) {
				this.outStream.write(this.buffer, 0, this.pos);
				this.writtenDown += this.pos;
				this.pos = 0;
			}
			...
			if (this.pos > 0) {
				this.outOutput.write(this.buffer, 0, this.pos);
				this.writtenDown += this.pos;
				this.pos = 0;
			}
			this.outOutput = null;
	...
	/** Flushes this buffered output stream. This forces any buffered output bytes to be written out
	...
	/** @return amount of bytes written since last setStream or setOutput method was called. */
	public final int getWrittenByteCount() {
		
		return this.writtenDown + this.pos;
	}

	/** Replace output stream. Supports both - wrapping DataOutput object and OutputStream object.
	 *
	 * @param out */
	public final void setOutput(final DataOutput out) {
	 ...
	 *
	 * @param out */
	public final void setStream(final OutputStream out) {
	...
	public void write(final byte b[], final int off, final int len) throws IOException {
		...
		if (len >= this.buffer.length - this.pos) {
			...
			if (len >= this.buffer.length) {
				/* If the request length exceeds the size of the output buffer, we should write the
				 * data directly. In this way buffered streams will cascade harmlessly. */
				if (this.outStream != null) {
					this.outStream.write(b, off, len);
				} else {
					this.outOutput.write(b, off, len);
				}
				this.writtenDown += len;
				return;
			}
		}
	...
	/** Writes out the string to the underlying output stream as a sequence of bytes. Each character
	 * in the string is written out, in sequence, by discarding its high eight bits. If no exception
	 * is thrown, the counter <code>written</code> is incremented by the length of <code>s</code>.
	 *
	 * @param s
	 ...
	 * first. If no exception is thrown, the counter <code>written</code> is incremented by
	 * <code>2</code>.
	 *
	 * @param v
	...
	/** Writes a string to the underlying output stream as a sequence of characters. Each character
	 * is written to the data output stream as if by the <code>writeChar</code> method. If no
	 * exception is thrown, the counter <code>written</code> is incremented by twice the length of
	 * <code>s</code> .
	 *
	 * @param s
	 ...
	 * underlying output stream as an 8-byte quantity, high byte first. If no exception is thrown,
	 ...
	 * underlying output stream as a 4-byte quantity, high byte first. If no exception is thrown,
	 * the counter <code>written</code> is incremented by <code>4</code>.
	 *
	 * @param v
	...
	/** Writes all bytes from given InputStream using own buffers to make this transfer more
	 * effective.
	 *
	 * @param stream
	...
	public int writeFullyFromStream(final InputStream stream) throws IOException {
		...
		int written = 0;
		for (;;) {
			if (this.buffer.length - this.pos < 1024) {
				this.flushBuffer();
			}
			final int read = stream.read(this.buffer, this.pos, this.buffer.length - this.pos);
			if (read == -1) {
				break;
			}
			this.pos += read;
	 ...
	 * If no exception is thrown, the counter <code>written</code> is incremented by <code>4</code>.
	 *
	 * @param v
	 ...
	 * In no exception is thrown, the counter <code>written</code> is incremented by <code>8</code>.
	 *
	 * @param v
	 ...
	 * not the length of the string. Following the length, each character of the string is output,
	 * in sequence, using the modified UTF-8 encoding for the character. If no exception is thrown,
	 * the counter <code>written</code> is incremented by the total number of bytes written to the
	 * output stream. This will be at least two plus the length of <code>str</code>, and at most two
	 * plus thrice the length of <code>str</code>.
	 *
	 * @param str
	...
	public final void writeUTF(final String str) throws IOException {
		...
		if (i == strlen) {
			...
			for (i = restart; i < strlen; ++i) {
				...
				if (c > 0x07FF) {
					if (this.buffer.length - this.pos < 3) {
						this.flushBuffer();
					}
