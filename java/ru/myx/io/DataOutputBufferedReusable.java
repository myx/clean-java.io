...
/**
 * Fast, highly buffered, contains additional control methods. Supports both -
 * wrapping DataOutput object and OutputStream object.
 * 
 * Special method - getWrittenByteCount() returns amount of bytes written since
 * last setStream or setOutput method was called.
 * 
 ...
 * 
 * UTF buffer used to try not to traverse string twice for calculating byte size
 * before flushing to stream.
 * 
 * @author myx
 * 
 *         BUFFER_CAPACITY = 64k
...
public final class DataOutputBufferedReusable extends OutputStream implements DataOutput {
	...
	private final byte[]		buffer			= new byte[DataOutputBufferedReusable.BUFFER_CAPACITY];
	
	private final byte[]		bytearr			= new byte[DataOutputBufferedReusable.BUFFER_UTF];
	...
	/**
	 * @return amount of bytes written since last setStream or setOutput method
	 *         was called.
	 */
	...
	/**
	 * Replace output stream. Supports both - wrapping DataOutput object and
	 * OutputStream object.
	 * 
	 * @param out
	 */
	...
	public void write(final byte b[], final int off, final int len) throws IOException {
		if (len >= DataOutputBufferedReusable.BUFFER_CAPACITY - this.pos) {
			this.flushBuffer();
			if (len >= DataOutputBufferedReusable.BUFFER_CAPACITY) {
				/*
				 * If the request length exceeds the size of the output buffer,
				 * we should write the data directly. In this way buffered
	...
	public void write(final int b) throws IOException {
		if (this.pos >= DataOutputBufferedReusable.BUFFER_CAPACITY) {
			this.flushBuffer();
		}
	...
	public final void writeChar(final int v) throws IOException {
		if (DataOutputBufferedReusable.BUFFER_CAPACITY - this.pos < 2) {
			this.flushBuffer();
		}
	...
	public int writeFullyFromStream(final InputStream stream) throws IOException {
		int written = 0;
		for (;;) {
			if (DataOutputBufferedReusable.BUFFER_CAPACITY - this.pos < 1024) {
				this.flushBuffer();
			}
			final int read = stream.read( this.buffer, this.pos, DataOutputBufferedReusable.BUFFER_CAPACITY - this.pos );
			if (read == -1) {
				break;
			}
			this.pos += read;
	...
	public final void writeInt(final int v) throws IOException {
		if (DataOutputBufferedReusable.BUFFER_CAPACITY - this.pos < 4) {
			this.flushBuffer();
		}
	...
	public final void writeLong(final long v) throws IOException {
		if (DataOutputBufferedReusable.BUFFER_CAPACITY - this.pos < 8) {
			this.flushBuffer();
		}
	...
	public final void writeUTF(final String str) throws IOException {
		...
		for (; i < strlen; ++i) {
			...
			if (c >= 0x0001 && c <= 0x007F) {
				if (DataOutputBufferedReusable.BUFFER_UTF - count < 1) {
					break;
				}
			...
			if (c > 0x07FF) {
				if (DataOutputBufferedReusable.BUFFER_UTF - count < 3) {
					break;
				}
			...
			if (DataOutputBufferedReusable.BUFFER_UTF - count < 2) {
				break;
			}
		...
		if (i == strlen) {
			...
			for (i = restart; i < strlen; ++i) {
				final char c = str.charAt( i );
				if (c >= 0x0001 && c <= 0x007F) {
					if (DataOutputBufferedReusable.BUFFER_CAPACITY - this.pos < 1) {
						this.flushBuffer();
					}
				...
				if (c > 0x07FF) {
					if (DataOutputBufferedReusable.BUFFER_CAPACITY - this.pos < 3) {
						this.flushBuffer();
					}
